...
멋쟁이사자처럼 클라우드 엔지니어링 부트캠프 2차 팀 프로젝트. 렌주룰 기반에 '증강' 요소를 더한 오목 게임을 기획부터 배포까지 직접 만들었다. **Node.js (TypeScript) 백엔드**와 **React (Vite + TypeScript) 프론트엔드**로 구성된 실시간 멀티플레이어 게임이다.

## 팀 구성 및 역할

- **김동재** (본인): 백엔드·프론트엔드 개발 주도, 게임 로직 개발
- **김다현**: 프론트엔드 설계, 백엔드 프로필 관리 로직 개발
- **조윤지**: 백엔드 프로필 관리 로직 개발, 프로젝트 문서화
- **양지훈**: 인프라 설계 및 관리

## 주요 기능

- 인증, 로비, 대기방(실시간 매칭·방 생성) → 실시간 오목 대전
- 렌주룰 + 증강 요소가 있는 오목 — 단순 클론이 아니라 게임성을 직접 기획해 추가
