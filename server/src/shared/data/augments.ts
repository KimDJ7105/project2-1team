...
export type AugmentRarity = 'COMMON' | 'RARE' | 'EPIC' | 'LEGEND';
...
export class Augment implements IAugment {
  ...
  constructor(id: string, name: string, description: string, type: AugmentType, rarity: AugmentRarity) {
    this.id = id;
    this.name = name;
...
export const AUGMENT_LIST: Augment[] = [
  new Augment('sniper', '저격', '상대 돌 1개 선택해 제거 (최근에 둔 돌 제거 불가)', 'TARGET_SELECT', 'RARE'),
  new Augment('seal_empty', '빈칸 봉인', '빈칸 하나 선택 시 다음 3턴 동안 누구도 그 칸에 돌을 둘 수 없게 합니다.', 'TARGET_SELECT', 'COMMON'),
  ...
  new Augment('steal', '도둑질', '상대 증강 1개를 훔쳐옵니다.', 'IMMEDIATE', 'EPIC'),
